...
export default function TablePagination({
  ...
  const indexOfLastSubClass = Math.min(
  ...
  useEffect(() => {
    setCurrentSubClass(
      subClass
        .filter((item) =>
          item[columnName].toLowerCase().includes(term.toLowerCase())
        )
        .slice(indexOfFirstSubClass, indexOfLastSubClass)
    );
  }, [subClass, term, indexOfFirstSubClass, indexOfLastSubClass, columnName]);
